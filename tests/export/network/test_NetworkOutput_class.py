...
class NetworkOutputTest( django.test.TestCase ):
    ...
    def test_render_network_data( self ):
        ...
        reference_data = None
        reference_data_char_count = None
        test_value = None
        should_be = None
        error_string = None
        ...
        # diff
        temp_data = network_data.strip()
        network_data_lines = temp_data.splitlines()
        temp_data = reference_data.strip()
        reference_data_lines = temp_data.splitlines()
        diff_count = 0
        for diff_output_line in difflib.unified_diff( network_data_lines,
                                                      reference_data_lines,
                                                      fromfile = 'rendered_data',
        ...
        # line counts?
        network_data_line_count = len( network_data_lines )
        reference_data_line_count = len( reference_data_lines )
        test_value = network_data_line_count
        should_be = reference_data_line_count
        error_string = "line counts are not equal. rendered: {}; reference: {}.".format( test_value, should_be )
        self.assertEqual( test_value, should_be, msg = error_string )

        # loop and compare line-by-line
        for line_index in range( network_data_line_count ):
        
            # get lines
            network_data_line = network_data_lines[ line_index ]
            reference_data_line = reference_data_lines[ line_index ]
                
            # lines should be the same
            test_value = network_data_line
